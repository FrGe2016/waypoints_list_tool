...
waypoints = []
starting_waypoint=0
ending_waypoint=-5      # Enter the value	-1 for all the list 
...
class FollowPath(State):
	...
	def execute(self, userdata):
		global waypoints , ending_waypoint, starting_waypoint
		# Execute waypoints each in sequence
		if (ending_waypoint)<0: #enter -1 to go to last waypoint
				ending_waypoint=len(waypoints)
		if (ending_waypoint)>len(waypoints):
				ending_waypoint=len(waypoints)
...
#		starting_waypoint=raw_input("Starting after last waypoint, enter a new value ? ")
		for waypoint in waypoints:
			...
			if (waypoints.index >= starting_waypoint and waypoints.index < ending_waypoint):
				goal = MoveBaseGoal()
...
def convert_PoseWithCovArray_to_Polygone(waypoints):
	# Used to publish waypoints as pose array so that you can see them in rviz, etc.
	poly = PolygonStamped()
...
#	print(poly)
	...
	"""
		for pose in waypoints:
		print(pose)
		#path.poses.header.frame_id = rospy.get_param('~goal_frame_id','map')
		#  path.poses.stamp = 	timestamp
		#  path.poses.poses=pose
		#  print(path)
	"""
...
class GetPath(State):
	...
	def display_waypoints_in_rviz(self):
		...
		self.polygone_publisher.publish(
			convert_PoseWithCovArray_to_Polygone(waypoints))
